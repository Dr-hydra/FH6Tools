...
Public Class ValidateFolderName
    ...
    Public Property UseStrictNameCharCheck As Boolean = True
    Public Property IgnoreCase As Boolean = True
    Private ReadOnly PathIgnore As IEnumerable(Of String)
    Public Sub New()
    End Sub
    Public Sub New(Folder As String, Optional UseStrictNameCharCheck As Boolean = True, Optional IgnoreCase As Boolean = True)
        Me.Folder = Folder
        Me.IgnoreCase = IgnoreCase
        Me.UseStrictNameCharCheck = UseStrictNameCharCheck
    ...
    Public Overrides Function Validate(Str As String) As String
        Try
            ...
            '检查特殊字符
            Dim CharactCheck As String = New ValidateExcept(Path.GetInvalidFileNameChars() & If(UseStrictNameCharCheck, "!;", ""), "文件夹名不可包含 % 字符！").Validate(Str)
...
Public Class ValidateFileName
    ...
    Public Sub New()
    End Sub
    Public Sub New(Name As String, Optional UseStrictNameCharCheck As Boolean = True, Optional IgnoreCase As Boolean = True)
        Me.Name = Name
        Me.IgnoreCase = IgnoreCase
        Me.UseStrictNameCharCheck = UseStrictNameCharCheck
    End Sub
    Public Overrides Function Validate(Str As String) As String
        Try
            ...
            '检查特殊字符
            Dim CharactCheck As String = New ValidateExcept(Path.GetInvalidFileNameChars() & If(UseStrictNameCharCheck, "!;", ""), "文件名不可包含 % 字符！").Validate(Str)
            If CharactCheck <> "" Then Return CharactCheck
            '检查特殊字符串
...
Public Class ValidateFolderPath
    ...
    Public Sub New()
    End Sub
    Public Sub New(UseStrictNameCharCheck As Boolean)
        Me.UseStrictNameCharCheck = UseStrictNameCharCheck
    End Sub
    Public Overrides Function Validate(Str As String) As String
        '去除尾部斜线，统一为 \
...
Fin:
        ...
        For i = If(Str.StartsWithF("\\Mac\"), 2, 1) To Str.Split("\").Count - 1
            ...
            '检查特殊字符
            Dim CharactCheck As String = New ValidateExcept(Path.GetInvalidFileNameChars() & If(UseStrictNameCharCheck, "!;", ""), "路径中存在无效字符！").Validate(SubStr)
