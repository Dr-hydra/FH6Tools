...
v2.2.0 增加了在每个官方 curated 工具卡片右上方单独“使用本地压缩包安装”的功能，支持通过 GitHub 官方最新的 `digest` 属性进行在线 SHA-256 校验和自动极速识别，同时支持解包检测 EXE 启动特征与版本提取，不影响第三方 Local 导入的原生功能。此外，修正了 Forza Painter FH6 的可执行文件通配符规则以支持包含版本号的 Release 文件名，在个性化设置中增加了自定义游戏图标与左栏 Logo 双向实时同步的快捷磁贴，并内置更新了 2026-06-18 赛季的攻略图与一图流网址。
