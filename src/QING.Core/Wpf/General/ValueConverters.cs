...
public class AdditionConverter : IValueConverter {
    ...
    public object? ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture) {
        if (value is null) return WpfBinding.DoNothing;
        if (!double.TryParse(value.ToString(), out double before)) return WpfBinding.DoNothing;
        double scale = 1;
        if (parameter is not null) double.TryParse(parameter.ToString(), out scale);
        return before - scale;
    }
...
public class MultiplicationConverter : IValueConverter {
    ...
    public object? ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture) {
        ...
        double scale = 1;
        if (parameter is not null) double.TryParse(parameter.ToString(), out scale);
        if (scale == 0) return WpfBinding.DoNothing;
        return before / scale;
    }
}
