...
- 从独立远程元数据清单同步工具名称、项目地址和中英简介，下载、安装、更新和卸载社区工具。
![alt text](image.png)
...
候选工具来源记录在 [tools.md](tools.md)。工具的下载方式、启动方式、管理员要求与风险等级由软件内置清单控制；独立远程清单仅能同步项目地址、名称和中英简介。FH6Tools 也允许用户添加本地工具。
想要添加工具欢迎提交issue。或者你可以自行导入。

## 升级说明

从 v1.1.0 或更早版本升级到 v1.1.1 时，请重新下载完整发布包，不要使用旧版本的应用内更新。v1.1.1 将工具运行定义改为本地受信任清单，并使用独立远程元数据清单。
